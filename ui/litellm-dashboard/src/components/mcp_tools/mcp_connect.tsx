...
const FeatureCard: React.FC<FeatureCardProps> = ({
  ...
  const getHeadersConfig = () => {
    const headers: Record<string, any> = {
      "x-Zentris-api-key": "Bearer YOUR_Zentris_API_KEY",
    };
    ...
    <Space direction="vertical" size="large" className="w-full">
      <div className="bg-gradient-to-r from-emerald-50 to-green-50 p-6 rounded-lg border border-emerald-100">
        <div className="flex items-center gap-3 mb-3">
          ...
          <Title level={4} className="mb-0 text-emerald-900">
            Zentris Proxy API Integration
          </Title>
        </div>
        <Text className="text-emerald-700">
          Connect to Zentris Proxy Responses API for seamless tool integration with multiple model providers
        </Text>
      </div>
      ...
      <Space direction="vertical" size="large" className="w-full">
        <FeatureCard
          ...
          description="Configure your Zentris Proxy Virtual Key for authentication"
        >
          <Space direction="vertical" size="middle" className="w-full">
            <div>
              <Text>Get your Virtual Key from your Zentris Proxy dashboard or contact your administrator</Text>
            </div>
            <CodeBlock title="Environment Variable" code='export Zentris_API_KEY="sk-..."' copyKey="Zentris-env" />
          </Space>
        </FeatureCard>
        ...
        <FeatureCard
          ...
          description="Connection details for your Zentris MCP server"
        >
          <CodeBlock title="Server URL" code={`${proxyBaseUrl}/mcp`} copyKey="Zentris-server-url" />
        </FeatureCard>

        <FeatureCard
          icon={<Code className="text-emerald-600" size={16} />}
          title="Implementation Example"
          description="Complete cURL example for using the Zentris Proxy Responses API"
...
--header "Authorization: Bearer $Zentris_VIRTUAL_KEY" \\
--data '{
    "model": "gpt-4",
    "tools": [
        {
            ...
            "headers": {
                "x-Zentris-api-key": "Bearer YOUR_Zentris_VIRTUAL_KEY",
    ...
    <Space direction="vertical" size="large" className="w-full">
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg border border-blue-100">
        ...
        <Text className="text-blue-700">
          Connect OpenAI Responses API to your Zentris MCP server for seamless tool integration
        </Text>
      </div>
    ...
    <Space direction="vertical" size="large" className="w-full">
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-6 rounded-lg border border-purple-100">
        ...
        <Text className="text-purple-700">
          Use tools directly from Cursor IDE with Zentris MCP. Enable your AI assistant to perform real-world tasks
    ...
    <Space direction="vertical" size="large" className="w-full">
      <div className="bg-gradient-to-r from-green-50 to-teal-50 p-6 rounded-lg border border-green-100">
        ...
        <Text className="text-green-700">
          Connect to Zentris MCP using HTTP transport. Compatible with any MCP client that supports HTTP streaming.
        </Text>
      </div>
      ...
      <Space direction="vertical" size="large" className="w-full">
        <div>
          ...
          <TremorText className="text-lg text-gray-600">
            Use tools directly from any MCP client with Zentris MCP. Enable your AI assistant to perform real-world

  ...
  app.post("/api/demo/run", async (_request, reply) => {
    ...
    const results: DemoResult[] = [
      ...
      },
      {
        ...
        summary: "The actual injection detector stopped a hierarchy-override attempt before model routing.",
      ...
      },
      {
        ...
        action: rag.dropped.length > 0 ? "BLOCK" : "ALLOW",
        risk: rag.dropped.length > 0 ? "high" : "low",
        summary: "The actual RAG guard evaluates untrusted chunks before adding them to model context.",
        signals: rag.dropped.map((chunk) => chunk.reason),
        owasp: "LLM01: Prompt Injection",
        mitre: "AML.T0051.001",
